...
var findIndex = function(arr, callback) {
	return arr.reduce(function(index, item, currentIndex) {
		if (index < 0 && callback(item, currentIndex))
			return currentIndex;
		return index;
	}, -1);
};
...
var removeAttributes = function(elem, atts) {
	atts.forEach(function(attribute) {
		...
		if (attribute.att === "class") {
			...
			removeStyles(
				elem,
				Array.prototype.slice.call(elem.style)
			);
		} else {
			if (attribute.att in elem) {
				try {
...
var diff = function(templateMap, domMap, elem, polyps) {
	...
	templateMap.forEach(function(node, index) {
		...
		if (
			templateMap[index].content &&
			templateMap[index].content !== domMap[index].content
		) {
			domMap[index].node.textContent =
				templateMap[index].content;
		}

		// If target element should be empty, wipe it
		if (
			domMap[index].children.length > 0 &&
		...
		if (
			domMap[index].children.length < 1 &&
			node.children.length > 0
		) {
			var fragment = document.createDocumentFragment();
			diff(
				node.children,
				domMap[index].children,
				fragment,
				polyps
			);
...
var renderPolyps = function(polyps, reef) {
	...
	polyps.forEach(function(coral) {
		if (coral.attached.indexOf(reef) > -1)
			return err(
				"ReefJS: " +
					reef.elem +
...
Component.prototype.render = function() {
	...
	// Get the template
	var template =
		trueTypeOf(this.template) === "function"
			? this.template(data)
			: this.template;
...
Component.prototype.setData = function(obj) {
	if (trueTypeOf(obj) !== "object")
		return err("ReefJS: The provided data is not an object.");
	for (var key in obj) {
		if (obj.hasOwnProperty(key)) {
